...
function checkUrl(a, b) {
	return a ? a.length < 20 || a.indexOf("ceshij.com") >= 0 || a.indexOf("127.1") >= 0 || a.indexOf("127.0.0.1") >= 0 ? !1 : !0 : b ? b : !1
}
function UrlRegEx(a) {
	var b = /(\w+): \/\/([^\:|\/]+)(\:\d*)?(.*\/)([^#|\?|\n]+)?(#.*)?(\?.*)?/i,
		c = a.match(b);
...
function tooltipimgLoaded() {
	$(".june-tooltipimg").each(function(a, b) {
		var c = getImgNaturalDimensions(b, function(a, b) {
			a && $(this).next("div.june-tooltipdiv").children("i").html(a + "X" + b)
		});
		c[0] && $(this).next("div.june-tooltipdiv").children("i").html(c[0] + "X" + c[1])
	})
}
function quickSort(a) {
	var b, c, d, e, f;
	if (a.length <= 1) return a;
	for (b = Math.floor(a.length / 2), c = a.splice(b, 1)[0], d = [], e = [], f = 0; f < a.length; f++) a[f] < c ? d.push(a[f]) : e.push(a[f]);
	return quickSort(d).concat([c], quickSort(e))
}
function IsURL(a) {
...
function defaultStyleConfigList() {}
function juneSimpleSlider() {}
var Sys = {},
	ua = navigator.userAgent.toLowerCase();
window.ActiveXObject ? Sys.ie = ua.match(/msie ([\d.]+)/)[1] : document.getBoxObjectFor ? Sys.firefox = ua.match(/firefox\/([\d.]+)/)[1] : window.MessageEvent && !document.getBoxObjectFor ? Sys.chrome = ua.match(/chrome\/([\d.]+)/) && ua.match(/chrome\/([\d.]+)/)[1] : window.opera ? Sys.opera = ua.match(/opera.([\d.]+)/)[1] : window.openDatabase && (Sys.safari = ua.match(/version\/([\d.]+)/)[1]), function(a) {
	a.fn.addGroup = function(b) {
		if (b && b.length > 0) {
			var c = this.toArray();
			return c.push(b[0]), a(c)
		}
		return a()
	}
}(jQuery), function(a) {
	...
	a.fn.tResize = function(d) {
		...
		var d, e, f, g, h, i;
		for (b.children(".ui-resizable-handle").remove(), d = {
			n: "",
			e: "",
			s: "",
			w: "",
