jQuery.base64 = function() {
	...
	_dom = null, ts_1 = {
		...
		var a = $(this),
			...
			}else if("gifzh"==b){$("#urlLabel").html("GIF图片地址：");
			}else{$("#urlLabel").html("PC端地址");}
			if("gifzh"==b){$("#urlSubmit").html("立即转换");$("#j_label").html("转换后地址:");}else{
				$("#urlSubmit").html("点击获取手机端链接");$("#j_label").html("手机端地址:");}
		...
		e = b.match(/category-(\d+)(?=(\.))/g), e && e.length > 0 ? g = e[0].replace(/category-/g, "") : (e = b.match(/((\&|\?){1}catid\=){1}(\d+)(?=(\&|$))/gi), e && e.length > 0 && (g = e[0].replace(/(\&|\?)catid\=/gi, "")))
      ...
      if (re.test(str_url)){
             $.ajax({
              ...
              var data=JSON.parse(data);
             	c="https://shop"+data.shopId+".m.taobao.com/#list?catId="+g;
             	j = "http://qr.liantu.com/api.php?bg=ffffff&fg=000000&gc=000000&el=l&w=" + k + "&m=10&text=" + c, $("#urlEwm").attr("src", j), $("#urlEwm1").attr("src", l);
               $("#fl1").val(c);
               $("#gm").attr("src", c);
               $("#fl2").val("https://shop"+data.shopId+".m.taobao.com/#list?catId="+g+"&sort=hotsell");
               $("#fl3").val("https://shop"+data.shopId+".m.taobao.com/#list?catId="+g+"&sort=oldstarts");
               $("#fl4").val("https://shop"+data.shopId+".m.taobao.com/#list?catId="+g+"&sort=isPromotion");
               $("#fl5").val("https://shop"+data.shopId+".m.taobao.com/#list?catId="+g+"&sort=_bid");
               $("#fl6").val("https://shop"+data.shopId+".m.taobao.com/#list?catId="+g+"&sort=bid");
            }
          });
      }
     }
    IsURL(url)
		...
		var pageData={"pUrlEle":$('#urlValue'),"murl":""}
        var url=pageData.pUrlEle.val();
        var data= {
            "action": "geturl",
            "url": url
        }
         var passToUrl='code.php';
         var patt1 = new RegExp("http");
         var result1 = patt1.test(url);
         var patt2 = new RegExp("gif");
         var result2 = patt2.test(url);
         if(result1&&result2){
            $.post(passToUrl,data,function(res){
                $('#urlResult').val(res);
                 pageData.murl=res;
            })
        }else{
