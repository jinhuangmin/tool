	...
	$.fn.smartMenu = function(data, options) {
		...
		var htmlCreateMenu = function(datum) {
						...
						$.each(arr, function(j, obj) {
							...
							if (text) {
								...
								if ($.isArray(obj.data) && obj.data.length) {
									htmlMenuLi = '<li class="'+ clKey +'li '+ _class +'" data-hover="true">' + htmlCreateMenu(obj.data) +
										'<i>'+obj.icon+'</i><a class="'+ clKey +'a"'+ strTitle +' data-key="'+ rand +'"><i class="'+ clKey +'triangle"></i>'+ text +'</a>' + 
									'</li>';
								} else {
									htmlMenuLi = '<li class="'+ clKey +'li '+ _class +'">' +
										'<i>'+obj.icon+'</i><a class="'+ clKey +'a"'+ strTitle +' data-key="'+ rand +'">'+ text +'</a>' + 
									'</li>';
								}
			...
			if (!jqueryMenu.length) {
				...
				//事件
				$(idKey + params.name).find("li").bind("click", function() {
