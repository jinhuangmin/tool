...
<a class="add fb" href="javascript:;" onclick=javascript:openwinx('?m=admin&c=zxn&a=add&menuid=&pc_hash=<?php echo $_SESSION['pc_hash'];?>','')><em><?php echo L('add_content');?></em></a>　
<span ID="tongbu" class="tongbu" style=" color:#0409E1; cursor:pointer; margin-left:20px;display: none;">写入初始数据</span>
<span style=" color:#999;display: none;">| 同步</span>
<span ID="delcj" class="delann" style=" color:#f00; cursor:pointer; margin-left:50px;display: none;">删除初始数据</span>
<span ID="deldiy" class="deldiy" style=" color:#f00; cursor:pointer;display: none;">删除自加数据</span>
<span ID="delall" class="delann" style=" color:#f00; cursor:pointer;display: none;">删除所有数据</span>
<span style=" color:#999;display: none;"> | 一点即删,绝不犹豫</span>
</div>
...
<tbody id="listul">
    ...
    <div class="btn"><label for="check_box">全选/取消</label>
    	...
    	<input type="button" class="button" value="排序" onclick="myform.action='?m=admin&c=zxn&a=listorder&dosubmit=1';myform.submit();"/>
		<input type="button" class="button" value="删除" onclick="myform.action='?m=admin&c=zxn&a=tbylDEL&dosubmit=1';return confirm_delete()"/>
			</div>
    <div id="pages"></div>
</div>
</form>
...
$("#tongbu").click(function(){
  $.ajax({
	 type: "post",
	 url: "?m=admin&c=complex&a=yldata&pc_hash=<?php echo $_SESSION['pc_hash'];?>",
...
$("#delcj").click(function(){
  $.ajax({
	 type: "post",
	 url: "?m=admin&c=zxn&a=ddd&pc_hash=<?php echo $_SESSION['pc_hash'];?>",
...
function getData(page){ 
	$.ajax({
		type: 'POST',
		url: '/index.php?m=admin&c=zxn&a=list_tbyl',
