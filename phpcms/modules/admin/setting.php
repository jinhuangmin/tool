...
class setting extends admin {
	 ...
	 * 保存配置信息
	 */
	public function zxtsave() {
		
		$zxn_set_db = pc_base::load_model('zxn_set_model');
		if(empty($_POST['setting']['zx_title'])){showmessage('标题为空你要闹那样?',HTTP_REFERER);}
		if(empty($_POST['setting']['zx_wang'])){showmessage('旺旺为空你不做生意了?',HTTP_REFERER);}
		if(empty($_POST['setting']['zx_copy'])){showmessage('版权为空!想什么呢',HTTP_REFERER);}
		if(empty($_POST['setting']['zx_style'])){showmessage('默认样式不能为空',HTTP_REFERER);}
		if(isset($_POST['setting']['zx_open'])){}else{showmessage('权限不能为空',HTTP_REFERER);}
		$update=$zxn_set_db->update(array('Title'=>$_POST['setting']['zx_title'],'Wang'=>$_POST['setting']['zx_wang'],'CopyRight'=>$_POST['setting']['zx_copy'],'Style'=>$_POST['setting']['zx_style'],'Open'=>$_POST['setting']['zx_open'],'Code'=>$_POST['setting']['zx_code']),array('Web'=>'tbtool'));
		showmessage(L('setting_succ'), HTTP_REFERER);

	}
	public function zxtinit() {
		
		$zxn_set_db = pc_base::load_model('zxn_set_model');
		
		if(empty($_POST['setting']['zx_index'])){showmessage('gaosunishouyehtmlbunengweikong,haiyongwojiang?',HTTP_REFERER);}
		if($_POST['setting']['zx_syhf']=='2'){
			$set = $zxn_set_db->get_one(array('Web'=>'tbtool'));
			$update=$zxn_set_db->update(array('IndexHtml'=>$set['IndexHtmlBF']),array('Web'=>'tbtool'));
			}
		if($_POST['setting']['zx_syhf']=='10'){
			$update=$zxn_set_db->update(array('IndexHtml'=>$_POST['setting']['zx_index']),array('Web'=>'tbtool'));
			}
