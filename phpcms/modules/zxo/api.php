...
class api {
	...
	public function saveInput () {
		...
		include template('zxo', "saveInput");
		}
	}
	...
	public function save() {
		...
		if($_POST['type']=='cover'){//系统自动保存
		  ...
		  if(!$r){//新写入自动保存
   $xie=$zxn_user_db->insert(array('UserName'=>$_username,'Title'=>$_POST['saveName'],'Width'=>'1920','Height'=>$_POST['saveObj']['actionCollection']['juneHeightVal'],'CreaTime'=>$time,'SaveTime'=>$time,'Cate'=>'','Value'=>$value,'Type'=>'2'),true);
   if(!$xie){
		$fanhui['0']=false;
		$fanhui['1']="保存失败0";
			}else{
	    ...
	    $up0=$zxn_user_db->update(array('UserName'=>$_username,'Title'=>$_POST['saveName'],'Width'=>'1920','Height'=>$_POST['saveObj']['actionCollection']['juneHeightVal'],'SaveTime'=>$time,'Cate'=>'','Value'=>$value,'Type'=>'2'),array('UserName'=>$_username,'Title'=>$_POST['saveName'])); 
	...
	public function load () {
		...
		$listinfos=$zxn_user_db->select(array('UserName'=>$_username,'Type'=>'2'));
		$arrlist = array();
		foreach($listinfos as $row){
			 $items = array();
			 $items['ID']=$row['ID'];
