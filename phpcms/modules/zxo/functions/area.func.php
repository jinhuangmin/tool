...
foreach($codeObj as $i=>$area){ 
   $config=$area['config'];
 if($area['appType']=='jdb'){
//  if(empty($area['pos'])){ $areapos='0|0|0|0';}else{$areapos=$area['pos'];}
   $pos=explode('|',$area['pos']);$scl='';
	...
	if($config['borderControl']=='hborder'){$bord='border:'.$config['borderWidth'].'px solid '.$config['borderColor'].';';$mtl='top:-1px;left:-1px;';}else{$bord='';$mtl='top:0px;left:0px;';}
	...
	$fanhui[$i]=' <div class="'.$area['appType'].' jspb abs '.$config['appID'].' '.$mongoliaMode.' '.$config['borderControl'].' '.$config['shadowControl'].'  '.$outlineWidth.' '.$outlclass.' '.$outlineSpeed.' '.$catr.'" title= "'.$config['tipText'].'" style="'.$sbj.''.$scl.''.$outline.''.$outlineShadowColor.'display:block;width:'.$pos['0'].'px;height:'.$pos['1'].'px;top:'.$pos['2'].'px;left:'.$pos['3'].'px;'.$shadow1.'z-index:10;" data-linkmode="'.$config['linkMode'].'" data-appid="'.$config['appID'].'" >
  ...
  $fanhui[$i]='<a class="'.$area['appType'].' abs '.$config['appID'].' '.$mongoliaMode.' '.$outlineWidth.'  '.$outlineSpeed.' '.$gzdiv.' '.$catr.'" '.$href.' '.$hrefMode.' style="'.$sbj.''.$scl.''.$outline.''.$outlineShadowColor.'display:block;width:'.$pos['0'].'px;height:'.$pos['1'].'px;top:'.$pos['2'].'px;left:'.$pos['3'].'px;z-index:10;" data-linkmode="'.$config['linkMode'].'" data-appid="'.$config['appID'].'" '.$brandid.'></a>';
  }
...
$lbzuhe1='<div class="jdhb J_TWidget abs '.$config['appID'].'" data-mr="'.$config['dtMode'].';'.$config['dtRange'].';'.$config['dtShowMode'].';'.$config['dtDirection'].'" data-widget-type="Carousel" data-widget-config="{"contentCls":"dhc'.$ls.'","navCls":"dhn'.$ls.'","effect":"'.$config['dtShowMode'].'","easing":"easeNone","duration":0.1, "delay":0, "autoplay":false,"activeIndex":'.$ksnum.'}" style="display:block;width:'.$pos['0'].'px;height:'.$pos['1'].'px;top:'.$pos['2'].'px;left:'.$pos['3'].'px;z-index:10;" data-linkmode="ptlink" data-appid="'.$config['appID'].'">
	<div class="scroller rel" style="overflow:hidden;width:'.$pos['0'].'px;height:'.$pos['1'].'px;z-index:10;">
		<div class="dhc'.$ls.'" style="position: absolute;">';
			...
			$cont=$cont.'<div class="abs" style="overflow:hidden;width:'.$pos['0'].'px;height:'.$pos['1'].'px;'.$sbj.''.$sbjc.''.$dis.'"></div>';	
				}
				
  $lbzuhe2='</div></div><ul class="abs dhn<?=$ls?>" style="top:0px;left:0px;z-index:12;background:none;">';
  $xwid=floor( $pos['0'] / $knum);$xgao=floor( $pos['1'] / $knum);
	...
	$ksnav=$ksnav.'<li class="abs '.$ks.'" style="width:'.$xwid.'px;height:'.$xgao.'px;top:'.$top.'px;left:'.$left.'px;"><a '.$href.' '.$hrefMode.' title="'.$config['tipText'].'" class="'.$catr.'" style="display:block;height:'.$xgao.'px;"></a></li>';
				} 

		
	$lbzuhe3='</ul></div>';

	$fanhui[$i]=$lbzuhe1.$cont.$lbzuhe2.$ksnav.$lbzuhe3;
	}
  
  
}
  $ok=preg_replace ( "/\s(?=\s)/","\\1", $fanhui );
 return $ok;
