...
class index extends foreground {
	...
	if($_POST['dosubmit']=='noyzm') {
		header('Content-Type:application/json;charset=utf-8');
		if($dec_text!='gNo'){$arr['success'] = 0; $arr['msg'] = '注册码错误'; echo json_encode($arr);exit();}
		$userinfo = array();
		$userinfo['license']=$_POST['license'] ;
		$userinfo['encrypt'] = create_randomstr(6);
		if((isset($_POST['username']) && is_username($_POST['username']))&&(preg_match("/^[\x{4e00}-\x{9fa5}A-Za-z0-9]{2,20}$/u",$_POST['username']))){$userinfo['username'] =$_POST['username'] ;}else{$arr['success'] = 0; $arr['msg'] = '用户名不正确，容汉字字母、数字（2-20位）'; echo json_encode($arr);exit();}
		if((isset($_POST['nickname']) && is_username($_POST['nickname']))&&(preg_match("/^[\x{4e00}-\x{9fa5}A-Za-z0-9]{2,20}$/u",$_POST['nickname']))){$userinfo['nickname'] =$_POST['nickname']; }else{$arr['success'] = 0; $arr['msg'] = '昵称不正确，容许汉字、字母、数字（2-20位）'; echo json_encode($arr);exit();}
		$userinfo['email'] = empty($_POST['email'])?''.$emls.'@nx.com':((isset($_POST['email']) && is_email($_POST['email'])) ? $_POST['email'] : exit('邮箱不正确'));
		if((isset($_POST['password']) && is_badword($_POST['password'])==false)&&(preg_match("/^[a-zA-Z0-9_]{6,20}$/u",$_POST['password']))){$userinfo['password'] =$_POST['password'] ;}else{$arr['success'] = 0; $arr['msg'] = '密码格式不正确）'; echo json_encode($arr);exit();}
		...
		if($member_detail->get_one(array('license'=>$userinfo['license']))) {$arr['success'] = 0; $arr['msg'] = '注册码已过期'; echo json_encode($arr);exit();}
		if($this->verify_db->get_one(array('username'=>$userinfo['username']))) {$arr['success'] = 0; $arr['msg'] = '用户名已存在,请更换或登录'; echo json_encode($arr);exit();}
		if($this->verify_db->get_one(array('nickname'=>$userinfo['nickname']))) {$arr['success'] = 0; $arr['msg'] = '昵称已存在'; echo json_encode($arr);exit();}
		if($this->verify_db->get_one(array('email'=>$userinfo['email']))) {$arr['success'] = 0; $arr['msg'] = '邮箱已存在'; echo json_encode($arr);exit();}
		$memberbd = pc_base::load_model('member_model');
		if($memberbd->get_one(array('username'=>$userinfo['username']))) {$arr['success'] = 0; $arr['msg'] = '用户名已存在,请更换或登录'; echo json_encode($arr);exit();}
		if($memberbd->get_one(array('nickname'=>$userinfo['nickname']))) {$arr['success'] = 0; $arr['msg'] = '昵称已存在'; echo json_encode($arr);exit();}
		if($memberbd->get_one(array('email'=>$userinfo['email']))) {$arr['success'] = 0; $arr['msg'] = '邮箱已存在'; echo json_encode($arr);exit();}	
			...
			$userinfo['license']=$_POST['license'] ;
			$userinfo['encrypt'] = create_randomstr(6);
			$userinfo['username'] = (isset($_POST['username']) && is_username($_POST['username']))&&(preg_match("/^[\x{4e00}-\x{9fa5}A-Za-z0-9]{2,20}$/u",$_POST['username'])) ? $_POST['username'] : exit('用户名不正确，容许汉字字母、数字（2-20位）');
			$userinfo['nickname'] = (isset($_POST['nickname']) && is_username($_POST['nickname']))&&(preg_match("/^[\x{4e00}-\x{9fa5}A-Za-z0-9]{2,20}$/u",$_POST['nickname'])) ? $_POST['nickname'] :  exit('昵称不正确，容许汉字、字母、数字（2-20位）');
			$userinfo['email'] = empty($_POST['email'])?''.$emls.'@nx.com':((isset($_POST['email']) && is_email($_POST['email'])) ? $_POST['email'] : exit('邮箱不正确'));
			$userinfo['password'] = (isset($_POST['password']) && is_badword($_POST['password'])==false)&&(preg_match("/^[a-zA-Z0-9_]{6,20}$/u",$_POST['password'])) ? $_POST['password'] : exit('密码格式不正确');
			$userinfo['modelid'] = isset($_POST['modelid']) ? intval($_POST['modelid']) : 10;
			$userinfo['regip'] = ip();
			$userinfo['point'] = $member_setting['defualtpoint'] ? $member_setting['defualtpoint'] : 0;
			$userinfo['amount'] = $member_setting['defualtamount'] ? $member_setting['defualtamount'] : 0;
		    ...
		    if($this->verify_db->get_one(array('username'=>$userinfo['username']))) {exit('用户名已存在,请更换或登录');}
			if($this->verify_db->get_one(array('nickname'=>$userinfo['nickname']))) {exit('昵称已存在');}
			if($this->verify_db->get_one(array('email'=>$userinfo['email']))) {exit('邮箱已存在');}
			$memberbd = pc_base::load_model('member_model');
		    if($memberbd->get_one(array('username'=>$userinfo['username']))) {exit('用户名已存在,请更换或登录');}
			if($memberbd->get_one(array('nickname'=>$userinfo['nickname']))) {exit('昵称已存在');}
			if($memberbd->get_one(array('email'=>$userinfo['email']))) {exit('邮箱已存在');}	
			}		
	...
	public function login() {
		...
		if($_GET['s']=='t3') {
				...
				if(isset($_POST['username']) ){$username = trim($_POST['username']);}else{
				$arr[0] = false;
				$arr[1] = $username;
				$arr[2] = '请输入用户名';
				header('Content-Type:application/json;charset=utf-8');
			...
			if(is_password($_POST['password']) && is_badword($_POST['password'])==false){
				trim($_POST['password']);
				}else{
				$arr[0] = false;
				$arr[1] = $username;
			...
			$this->message_db = pc_base::load_model('message_model');
			$msgdatas = $this->message_db->select(array('send_to_id'=>$username,'status'=>1));
			$arr[0] = true;
			$arr[1] = $username;
			$arr[2] = '登录成功';
			$arr['success'] = 1;
			$arr['username'] = $username;
			$arr['msg'] = '登录成功';//urlencode($synloginstr);
...
public function loginorout() {
		$c_username = param::get_cookie('_username');
		if($_POST['type']=='login'&&$_POST['username']==$c_username&&!empty($_POST['username'])){echo'ok';}else{echo'no';}
