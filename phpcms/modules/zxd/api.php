...
class api {
	...
	public function saveInput () {
		...
		$listinfos=$zxd_user_db->select(array('UserName'=>$_username));
		$arrlist = array();
		...
		$items='';
		include template('zxd', "saveInput");
		}
	}
	...
	public function save() {
		...
		if($_POST['type']=='cover'){//系统自动保存
		  $r = $zxd_user_db->get_one(array('UserName'=>$_username,'Title'=>$_POST['saveName']));
		  if(!$r){//新写入自动保存
   $xie=$zxd_user_db->insert(array('UserName'=>$_username,'Title'=>$_POST['saveName'],'Width'=>$width,'Height'=>$_POST['saveObj']['actionCollection']['juneHeightVal'],'CreaTime'=>$time,'SaveTime'=>$time,'Cate'=>'','Value'=>$value),true);
   if(!$xie){
		$fanhui['0']=false;
		$fanhui['1']="保存失败0";
			}else{
	    ...
	    $up0=$zxd_user_db->update(array('UserName'=>$_username,'Title'=>$_POST['saveName'],'Width'=>'1920','Height'=>$_POST['saveObj']['actionCollection']['juneHeightVal'],'SaveTime'=>$time,'Cate'=>'','Value'=>$value),array('UserName'=>$_username,'Title'=>$_POST['saveName'])); 
	...
	if($r){ 
		...
		$xie=$zxd_user_db->insert(array('UserName'=>$_username,'Title'=>$_POST['saveName'],'Width'=>'1920','Height'=>$_POST['saveObj']['actionCollection']['juneHeightVal'],'CreaTime'=>$time,'SaveTime'=>$time,'Cate'=>'','Value'=>$value),true);
		if(!$xie){
		$fanhui['0']=false;
		$fanhui['1']="保存失败";
			}else{
	...
	public function load() {
		...
		if($_POST['type']=='search'){  //查询
			$r = $zxd_user_db->get_one(array('ID'=>$_POST['ID'],'UserName'=>$_username));
			$fanhui = array();
       ...
       if($_POST['type']=='delete'){  //删除
			$r = $zxd_user_db->delete(array('ID'=>$_POST['ID'],'UserName'=>$_username));
			if(!$r){
			$fanhui['0']=false;
			$fanhui['1']="删除失败";
				}else{
	...
	public function gii () {
		...
		$poguo=file_get_contents('/zxn/form/geti/&items='.$ids.'&callback='.$callback.'&ewmSize='.$ewmSize.'');
		}
	}
	...
	public function useridata () {
		...
		$rows3=$zxn_cang_db->select(array('username'=>$_username,'type'=>'jd'));
		$arr=array();
...
public function insertjdcode () {
		if (isset($_POST)) {
		$zxd_linshi_db = pc_base::load_model('zxd_linshi_model');
		$_username = param::get_cookie('_username');
		$_usernick = param::get_cookie('_nickname');
		...
		$title='fx_title';
		$fx='0';
		$rnum=$zxd_linshi_db->select(array('UserName'=>$_username,'Fx'=>0),'*','','ID DESC');
		if(count($rnum)>9){  
		for($i=9;$i<count($rnum);$i++){
		  $delete=$zxd_linshi_db->delete(array('UserName'=>$_username,'ID'=>$rnum[$i]['ID']));
			}
		 }
		$inid=$zxd_linshi_db->insert(array('UserName'=>$_username,'Title'=>$title,'shareType'=>$_POST['shareType'],'Value'=>$code,'CreaTime'=>$time,'Fx'=>0),true);
		echo $inid;
	    }
	}	
...
public function like () {
		...
		$_usernick = param::get_cookie('_nickname');
		if($_POST['type']=='like'){
		   $r=$zxn_cang_db->get_one(array('type'=>'jd','username'=>$_username,'fxid'=>$_POST['ID']));
		   if(!$r){
			 $insert=$zxn_cang_db->insert(array('type'=>'jd','username'=>$_username,'fxid'=>$_POST['ID'],'author'=>$_POST['author'],'imgname'=>$_POST['imgname'],'title'=>$_POST['title']));
			 $numupdate=$zxd_ku_db->update(array('Cang'=>'+=1', ),array('ID'=>$_POST['ID']));
			 if($insert){$arr[0]=true;  $arr[1]="保存成功";}else{$arr[0]=false;  $arr[1]="保存失败";}
			   }
			 }
		if($_POST['type']=='unlike'){
		   $r=$zxn_cang_db->delete(array('type'=>'jd','username'=>$_username,'fxid'=>$_POST['ID']));
		   if($r){
			 $numupdate=$zxd_ku_db->update(array('Cang'=>'-=1', ),array('ID'=>$_POST['ID']));
...
public function server () {
     if (isset($_POST)) {
		...
		$rows=$zxn_cang_db->select(array('username'=>$_username,'type'=>'jd'));
		$rows3=$zxn_cang_db->listinfo(array('username'=>$_username,'type'=>'jd'), '', $page, '9');
		$arr=array();
		foreach($rows3 as $row){
		  $arr['ID']=$row['fxid'];
		  $arr['author']=$row['author'];
