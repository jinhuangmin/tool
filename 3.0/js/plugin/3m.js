...
function(a) {
	...
	function i(b, c, d, e) {
		var h, i, j, k, l, m, n, g = [];
		for (h = 0; h < b.length; h++) i = b[h], i ? (j = tinycolor(i), k = j.toHsl().l < .5 ? "sp-thumb-el sp-thumb-dark" : "sp-thumb-el sp-thumb-light", k += tinycolor.equals(c, i) ? " sp-thumb-active" : "", l = j.toString(e.preferredFormat || "rgb"), m = f ? "background-color:" + j.toRgbString() : "filter:" + j.toFilter(), g.push('<span title="' + l + '" data-color="' + j.toRgbString() + '" class="' + k + '"><span class="sp-thumb-inner" style="' + m + ';" /></span>')) : (n = "sp-clear-display", g.push(a("<div />").append(a('<span data-color="" style="background-color:transparent;" class="' + n + '"></span>').attr("title", e.noColorSelectedText)).html()));
	...
	function l(c, l) {
		function xb() {
			var b, c, d;
			if (n.showPaletteOnly && (n.showPalette = !0), kb.text(n.showPaletteOnly ? n.togglePaletteMoreText : n.togglePaletteLessText), n.palette) for (M = n.palette.slice(0), N = a.isArray(M[0]) ? M : [M], O = {}, b = 0; b < N.length; b++) for (c = 0; c < N[b].length; c++) d = tinycolor(N[b][c]).toRgbString(), O[d] = !0;
			X.toggleClass("sp-flat", p), X.toggleClass("sp-input-disabled", !n.showInput), X.toggleClass("sp-alpha-enabled", n.showAlpha), X.toggleClass("sp-clear-enabled", wb), X.toggleClass("sp-buttons-disabled", !n.showButtons), X.toggleClass("sp-palette-buttons-disabled", !n.togglePaletteOnly), X.toggleClass("sp-palette-disabled", !n.showPalette), X.toggleClass("sp-palette-only", n.showPaletteOnly), X.toggleClass("sp-initial-disabled", !n.showInitial), X.addClass(n.className).addClass(n.containerClassName), Ub()
		}
		function yb() {
			function c(b) {
				return b.data && b.data.ignore ? (Nb(a(b.target).closest(".sp-thumb-el").data("color")), Qb()) : (Nb(a(b.target).closest(".sp-thumb-el").data("color")), Qb(), Tb(!0), n.hideAfterPaletteSelect && Lb()), !1
			}
			var b, d;
			e && X.find("*:not(input)").attr("unselectable", "on"), xb(), nb && V.after(ob), wb || ib.hide(), p ? V.after(X).hide() : (b = "parent" === n.appendTo ? V.parent() : a(n.appendTo), 1 !== b.length && (b = a("body")), b.append(X)), zb(), a(T).on("click.spectrum touchstart.spectrum keyup.spectrum change.spectrum", ".pagecolor, .tColorPicker", function(b) {
				if (V != a(this) && Lb(), pb = V = a(this), !V.hasClass("disabled")) {
					var c = V.val();
					Nb(c), V.prev().children(".sp-preview").children(".sp-preview-inner").css("background-color", c), W || Hb(), w.move(c, V), a(b.target).is("input") || (b.stopPropagation(), b.preventDefault())
				}
				...
				if (!(V && V.length > 0 && V.hasClass("disabled"))) {
					var c = a(this);
					c.prev().val(""), V != c.prev() && Lb(), pb = V = c.prev(), V.prev().children(".sp-preview").children(".sp-preview-inner").css("background-color", "transparent"), w.move("", V)
				}
			}), V && (V.is(":disabled") || n.disabled === !0) && Yb(), X.click(o), eb.change(Gb), eb.bind("paste", function() {
				setTimeout(Gb, 1)
			}), eb.keydown(function(a) {
				13 == a.keyCode && Gb()
			}), hb.text(n.cancelText), hb.bind("click.spectrum", function(a) {
				a.stopPropagation(), a.preventDefault(), Mb(), Lb()
			}), ib.attr("title", n.clearText), ib.bind("click.spectrum", function(a) {
				a.stopPropagation(), a.preventDefault(), vb = !0, Qb(), p && Tb(!0)
			}), jb.text(n.chooseText), jb.bind("click.spectrum", function(a) {
				a.stopPropagation(), a.preventDefault(), e && eb.is(":focus") && eb.trigger("change"), Pb() && (Tb(!0), Lb())
			}), kb.text(n.showPaletteOnly ? n.togglePaletteMoreText : n.togglePaletteLessText), kb.bind("click.spectrum", function(a) {
				a.stopPropagation(), a.preventDefault(), V && V.length > 0 && V.hasClass("disabled") || (n.showPaletteOnly = !n.showPaletteOnly, n.showPaletteOnly || p || X.css("left", "-=" + (Y.outerWidth(!0) + 5)), xb())
			}), q(cb, function(a, b, c) {
				L = a / F, vb = !1, c.shiftKey && (L = Math.round(10 * L) / 10), Qb()
			}, Eb, Fb), q(_, function(a, b) {
				I = parseFloat(b / D), vb = !1, n.showAlpha || (L = 1), Qb()
			}, Eb, Fb), q(Z, function(a, b, c) {
				var d, e, f, g, h;
				c.shiftKey || "zx" != hre ? S || (d = J * A, e = B - K * B, f = Math.abs(a - d) > Math.abs(b - e), S = f ? "x" : "y") : S = null, g = !S || "x" === S, h = !S || "y" === S, g && (J = parseFloat(a / A)), h && (K = parseFloat((B - b) / B)), vb = !1, n.showAlpha || (L = 1), Qb()
			}, Eb, Fb), rb && "zx" == hre ? (Nb(rb), Rb(), tb = n.preferredFormat || tinycolor(rb).format, Ab(rb)) : Rb(), p && Ib(), d = e ? "mousedown.spectrum" : "click.spectrum touchstart.spectrum", fb.delegate(".sp-thumb-el", d, c), gb.delegate(".sp-thumb-el:nth-child(1)", d, {
				ignore: !0
			}, c)
		}
		function zb() {
		...
		function Ab(b) {
			if (t) {
				...
				if (!O[c] && -1 === a.inArray(c, P)) for (P.push(c); P.length > Q;) P.shift();
		...
		function Bb() {
			var b, c, a = [];
			if (n.showPalette) for (b = 0; b < P.length; b++) c = tinycolor(P[b]).toRgbString(), O[c] || a.push(P[b]);
		...
		function Cb() {
			...
			zb(), P && c.push(i(Bb(), b, "sp-palette-row sp-palette-row-selection", n)), fb.html(c.join(""))
		}
		function Db() {
			var a, b;
		...
		function Eb() {
			(0 >= B || 0 >= A || 0 >= D) && Ub(), z = !0, X.addClass(R), S = null, V.trigger("dragstart.spectrum", [Ob()])
		}
		function Fb() {
			z = !1, X.removeClass(R), V.trigger("dragstop.spectrum", [Ob()])
		}
		function Gb() {
			var b, a = eb.val();
		...
		function Nb(a, b) {
			if (tinycolor.equals(a, Ob())) return Rb(), void 0;
			var c, d;
			!a && wb ? vb = !0 : (vb = !1, c = tinycolor(a), d = c.toHsv(), I = d.h % 360 / 360, J = d.s, K = d.v, L = d.a), Rb(), c && c.isValid() && !b && (tb = n.preferredFormat || c.getFormat())
		}
		function Ob(a) {
		...
		function Qb() {
			if (!V.hasClass("disabled") && "zx" == hre) {
				Rb();
				var a = Ob();
				w.move(a, V), V.val(a), V.prev().children(".sp-preview").children(".sp-preview-inner").css("background-color", a.toString()), V.trigger("move.spectrum", [a])
			}
		}
		function Rb() {
			var a, b, c, d, g, h, i, j, k;
			eb.removeClass("sp-validation-error"), Sb(), a = tinycolor.fromRatio({
				h: I,
				s: 1,
				v: 1
			}), Z.css("background-color", a.toHexString()), b = tb, 1 > L && (0 !== L || "name" !== b) && ("hex" === b || "hex3" === b || "hex6" === b || "name" === b) && (b = "rgb"), c = Ob({
				format: b
			}), d = "", qb.removeClass("sp-clear-display"), qb.css("background-color", "transparent"), !c && wb && "zx" == hre ? qb.addClass("sp-clear-display") : (g = c.toHexString(), h = c.toRgbString(), f || 1 === c.alpha ? qb.css("background-color", h) : (qb.css("background-color", "transparent"), qb.css("filter", c.toFilter())), n.showAlpha && (i = c.toRgb(), i.a = 0, j = tinycolor(i).toRgbString(), k = "linear-gradient(left, " + j + ", " + g + ")", e ? bb.css("filter", tinycolor(j).toFilter({
				gradientType: 1
			}, g)) : (bb.css("background", "-webkit-" + k), bb.css("background", "-moz-" + k), bb.css("background", "-ms-" + k), bb.css("background", "linear-gradient(to right, " + j + ", " + g + ")"))), d = c.toString(b)), n.showInput && "zx" == hre && eb.val(d), n.showPalette && Cb(), Db()
		}
		function Sb() {
			var c, d, e, f, a = J,
				b = K;
		...
		function Tb(a) {
			...
			b && (c = b.toString(tb), Ab(b)), lb && V.val(c), a && d && (w.change(b), V.trigger("change", [b]))
		}
		function Ub() {
		...
		function Vb() {
			V.show(), a(T).off("click.spectrum touchstart.spectrum keyup.spectrum change.spectrum click.spectrump touchstart.spectrump click.spectrumc"), X.remove(), ob.remove(), d[$b.id] = null
		}
		function Wb(c, d) {
			return c === b && "zx" == hre ? a.extend({}, n) : d === b ? n[c] : (n[c] = d, "preferredFormat" === c && (tb = n.preferredFormat), xb(), void 0)
		}
		function Xb() {
			W = !1, V.attr("disabled", !1), pb.removeClass("sp-disabled")
		}
		function Yb() {
			Lb(), W = !0, V.attr("disabled", !0), pb.addClass("sp-disabled")
		}
		function Zb(a) {
			n.offset = a, Ub()
		}
	...
	function m(b, c) {
		var d = 0,
			...
			l = c.offset();
		return l.top += g + 4, l.left -= Math.min(l.left, l.left + e > j && j > e ? Math.abs(l.left + e - j) : 0), l.top -= Math.min(l.top, l.top + f > k && k > f ? Math.abs(f + g - d) : d), l
	}
	function n() {}
	function o(a) {
		a.stopPropagation()
	...
	function q(b, c, d, f) {
		...
		function o(a) {
			...
			if (h) {
				...
				d = a.originalEvent && a.originalEvent.touches && a.originalEvent.touches[0], f = d && d.pageX || a.pageX, m = d && d.pageY || a.pageY, o = Math.max(0, Math.min(f - i.left, k)), p = Math.max(0, Math.min(m - i.top, j)), l && n(a), c.apply(b, [o, p, a])
			}
		}
		function p(c) {
		...
		function() {}, d = d ||
		function() {}, f = f ||
		function() {}, g = document, h = !1, i = {}, j = 0, k = 0, l = "ontouchstart" in window, m = {}, m["selectstart"] = n, m["dragstart"] = n, m["touchmove mousemove"] = o, m["touchend mouseup"] = q, a(b).bind("touchstart mousedown", p)
	}
	function r(a, b, c) {
		var d;
		return function() {
	...
	var c = {
		...
		h = function() {
			var b, a = "";
			if (e) for (b = 1; 6 >= b; b++) a += "<div class='sp-" + b + "'></div>";
	...
	a.fn.spectrum = function(b) {
		...
		return "string" == typeof b ? (e = this, f = Array.prototype.slice.call(arguments, 1), this.each(function() {
			var g, c = d[a(this).data(t)];
			if (c) {
				if (g = c[b], !g) throw new Error("Spectrum: no such method: '" + b + "'");
		...
		function l(a, b, c) {
			var d, e, h, i, j, k;
			if (a = I(a, 255), b = I(b, 255), c = I(c, 255), d = g(a, b, c), e = f(a, b, c), j = (d + e) / 2, d == e) h = i = 0;
			else {
		...
		function m(a, b, c) {
			...
			var d, e, f, h, i;
			return a = I(a, 360), b = I(b, 100), c = I(c, 100), 0 === b ? d = e = f = c : (h = .5 > c ? c * (1 + b) : c + b - c * b, i = 2 * c - h, d = g(i, h, a + 1 / 3), e = g(i, h, a), f = g(i, h, a - 1 / 3)), {
				r: 255 * d,
				g: 255 * e,
				b: 255 * f
			}
		...
		function n(a, b, c) {
			var d, e, h, i, j, k;
			if (a = I(a, 255), b = I(b, 255), c = I(c, 255), d = g(a, b, c), e = f(a, b, c), j = d, k = d - e, i = 0 === d ? 0 : k / d, d == e) h = 0;
			else {
				switch (d) {
				case a:
					h = (b - c) / k + (c > b ? 6 : 0);
		...
		function C(a, b, c) {
			var d, e, f;
			for (b = b || 6, c = c || 30, d = i(a).toHsl(), e = 360 / c, f = [i(a)], d.h = (d.h - (e * b >> 1) + 720) % 360; --b;) d.h = (d.h + e) % 360, f.push(i(d));
			return f
		}
		function D(a, b) {
			var c, d, e, f, g, h;
			for (b = b || 6, c = i(a).toHsv(), d = c.h, e = c.s, f = c.v, g = [], h = 1 / b; b--;) g.push(i({
				h: d,
				s: e,
				v: f
			})), f = (f + h) % 1;
		...
		function S(c) {
			var d, e;
			if (c = c.replace(a, "").replace(b, "").toLowerCase(), d = !1, E[c]) c = E[c], d = !0;
		...
		i.prototype = {
			...
			toFilter: function(a) {
				var e, b = "#" + q(this._r, this._g, this._b, this._a),
					c = b,
					d = this._gradientType ? "GradientType = 1, " : "";
				return a && (e = i(a), c = e.toHex8String()), "progid:DXImageTransform.Microsoft.gradient(" + d + "startColorstr=" + b + ",endColorstr=" + c + ")"
			},
			toString: function(a) {
				var c, d, e, b = !! a;
				return a = a || this._format, c = !1, d = this._a < 1 && this._a >= 0, (e = !b && d && ("hex" === a || "hex6" === a || "hex3" === a || "name" === a)) ? "name" === a && 0 === this._a ? this.toName() : this.toRgbString() : ("rgb" === a && (c = this.toRgbString()), "prgb" === a && (c = this.toPercentageRgbString()), ("hex" === a || "hex6" === a) && (c = this.toHexString()), "hex3" === a && (c = this.toHexString(!0)), "hex8" === a && (c = this.toHex8String()), "name" === a && (c = this.toName()), "hsl" === a && (c = this.toHslString()), "hsv" === a && (c = this.toHsvString()), c || this.toHexString())
			},
			...
			if ("object" == typeof a) {
				c = {};
				for (d in a) a.hasOwnProperty(d) && (c[d] = "a" === d ? a[d] : O(a[d]));
				a = c
			}
			return i(a, b)
			...
			var d, e, f, g, h, j, k, l;
			return c = 0 === c ? 0 : c || 50, d = i(a).toRgb(), e = i(b).toRgb(), f = c / 100, g = 2 * f - 1, h = e.a - d.a, j = -1 == g * h ? g : (g + h) / (1 + g * h), j = (j + 1) / 2, k = 1 - j, l = {
				r: e.r * j + d.r * k,
				g: e.g * j + d.g * k,
				b: e.b * j + d.b * k,
				a: e.a * f + d.a * (1 - f)
			...
			var f, g, h, j, c = null,
				d = 0,
				e = !1;
			for (f = 0; f < b.length; f++) g = i.readability(a, b[f]), h = g.brightness > 125 && g.color > 500, j = 3 * (g.brightness / 125) + g.color / 500, (h && !e || h && e && j > d || !h && !e && j > d) && (e = h, d = j, c = i(b[f]));
			return c
		}, E = i.names = {
			aliceblue: "f0f8ff",
			antiquewhite: "faebd7",
