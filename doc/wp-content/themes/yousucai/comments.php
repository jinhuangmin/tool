...
<div class="pinglun" id="respond">
  ...
  <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform" name="commentform">
    <?php if($user_ID): ?>
    <p><?php printf(__('当前用户：%s　'),'<a href="'.get_option('siteurl').'/wp-admin" target="_blank"><font color="#eb5753">'.$user_identity.'</font></a>'); ?><a href="<?php echo wp_logout_url(get_permalink()); ?>" title="<?php _e('退出登录') ?>"><?php _e('退出登录'); ?></a></p>
    <?php else: ?>
	<br />
