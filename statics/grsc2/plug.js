...
function setUser(a) {
	a[0] ? ($.cookie("tLogin", !0), $.cookie("tLoginName", a[1]), JUNEZX__USERSTATUS = !0, JUNEZX__USERNAME = a[1], $("#nologin").hide(), $("#logged").show(), $("#userInfo").html(JUNEZX__USERNAME), $("#mainPic1").children("a").attr("href", "/3.0/"), $("#mainPic1").attr("data-cur", "3.0"), $.ajax({
		...
		success: function(b) {
			...
			if (b) {
				for (c = "", Math.ceil(b.total / 10), f = b.data.length, g = 0, h = b.saveData.length, i = b.collectData.length, j = 0, k = 0; f > k; k++) l = b.data[k].CreatTime.split(" "), m = l[0].split("-"), n = l[1].split(":"), o = new Date(m[0], m[1] - 1, m[2], n[0], n[1], n[2]).getTime(), (!e || e - o > 6e5) && 10 > g && (c += '<div class="lg-part" data-ID="' + b.data[k].ID + '">' + '<div class="lg-time"><span class="lg-time-label">生成时间</span><span class="lg-time-text">' + b.data[k].CreatTime + '</span><span class="lg-time-use" title="应用"><a href="/3.0/index#i_' + b.data[k].ID + '" target="_blank"></a></span><span class="lg-time-preview" title="预览"><a href="/3.0/previewP' + b.data[k].ID + '" target="_blank"></a></span></div>' + "</div>", e = o, g++);
				for ($("#userInfo_genList").html(c + '<div class="lg-last"><span class="lg-more" id="genMore">查看更多</span><div id="genMoreDialog" style="display:none;"></div></div>'), c = "", h > 10 && (h = 10), k = 0; h > k; k++) l = b.saveData[k].saveTime.split(" "), m = l[0].split("-"), n = l[1].split(":"), p = new Date(m[0], m[1] - 1, m[2], n[0], n[1], n[2]), q = p.Format("yyyy-MM-dd"), c += '<div class="lg-part" data-ID="' + b.saveData[k].ID + '">' + '<div class="lg-time">' + '<span class="lg-time-label">' + b.saveData[k].saveName + "</span>" + '<span class="lg-time-text">' + q + "</span>" + '<span class="lg-time-use" title="应用"><a href="/3.0/index#s_' + b.saveData[k].ID + '" target="_blank"></a></span>' + '<span class="lg-time-preview" title="预览"><a href="/3.0/previewZ' + b.saveData[k].ID + '" target="_blank"></a></span>' + "</div>" + "</div>";
				for ($("#userInfo_saveList").html(c + '<div class="lg-last"><span class="lg-more" id="saveMore">查看更多</span></div><div id="saveMoreDialog" style="display:none;"></div>'), c = "", k = 0; i > k; k++) 10 > j && (c += '<div class="lg-part" data-ID="' + b.collectData[k].ID + '">' + '<div class="lg-time"><span class="lg-time-img"><img src="' + b.collectData[k].url + '"></span><span class="lg-time-label">分享人</span><span class="lg-time-text">' + b.collectData[k].author + '</span><span class="lg-time-use" title="应用"><a href="/3.0/index#i_' + b.collectData[k].ID + '" target="_blank"></a></span><span class="lg-time-preview" title="预览"><a href="/3.0/previewP' + b.collectData[k].ID + '" target="_blank"></a></span></div>' + "</div>", j++), JUNEZX__USERCOLLECT.push(b.collectData[k].ID), setSUnLike(b.collectData[k].ID);
				$("#userInfo_faverList").html(c + '<div class="lg-last"><span class="lg-more" id="faverMore">查看更多</span><div id="faverMoreDialog" style="display:none;"></div></div>'), $("#genMore").on("click", function() {
					...
					tAjaxDialog(600, "saveMoreDialog", "保存列表", "/grsc/userList.php", {
						...
						if (a) {
							for (d = "", e = Math.ceil(b.total / 10), g = b.data.length, console.log(e), h = 0; g > h; h++) i = b.data[h].saveTime.split(" "), j = i[0].split("-"), k = i[1].split(":"), l = new Date(j[0], j[1] - 1, j[2], k[0], k[1], k[2]), m = l.Format("yyyy-MM-dd"), d += '<div class="lg-more-part" data-ID="' + b.data[h].ID + '">' + '<div class="lg-more-box">' + '<span class="lg-more-label">' + b.data[h].saveName + "</span>" + '<span class="lg-more-text">' + m + "</span>" + '<span class="lg-more-use sxu2" title="应用"><a href="/3.0/index#s_' + b.data[h].ID + '" target="_blank"></a></span>' + '<span class="lg-more-preview sxp2" title="预览"><a href="/3.0/previewZ' + b.data[h].ID + '" target="_blank"></a></span>' + "</div>" + "</div>";
					...
					tAjaxDialog(800, "faverMoreDialog", "收藏列表", "/grsc/userList.php", {
						...
						var d, e, f, g;
						if (a) {
							for (d = "", e = b.data.length, f = Math.ceil(e / 8),
							 g = 0; e > g; g++) d += '<div class="lg-more-part" data-ID="' + b.data[g].ID + '">' + '<div class="lg-more-box"><span class="lg-more-img"><img src="/grsc/images/s.gif" data-lsrc="' + b.data[g].url + '" /></span><span class="lg-more-label">分享人</span><span class="lg-more-text">' + b.data[g].author + '</span><span class="lg-more-use sxu1" title="应用"><a href="/3.0/index#i_' + b.data[g].ID + '" target="_blank"></a></span><span class="lg-more-preview sxp1" title="预览"><a href="/3.0/previewP' + b.data[g].ID + '" target="_blank"></a></span></div>' + "</div>";
	...
	a.html(b), $("#userListSearchKey").val((new Date).Format("yyyy-MM-dd")), $('<div class="closedialogbtn">关闭</div>').on("click", function() {
		...
		return "" == b ? (D.tMessage("请输入一个有效日期进行查询!"), void 0) : (tAjaxSearch("/grsc/userList.php", {
			...
			var g, h, c = "",
				...
				f = a.data.length;
			for (g = 0; f > g; g++) c += '<div class="lg-more-part" data-ID="' + a.data[g].ID + '">' + '<div class="lg-more-box"><span class="lg-more-label">生成时间</span><span class="lg-time-text">' + a.data[g].CreatTime + '</span><span class="lg-more-use" title="应用"><a href="/3.0/index#i_' + a.data[g].ID + '" target="_blank"></a></span><span class="lg-time-preview" title="预览"><a href="/3.0/previewP.php?ID=' + a.data[g].ID + '" target="_blank"></a></span></div>' + "</div>";
